...
const additionalColumns = computed(() => {
  const cols = [];
  if (roles.value) {
    cols.push({ name: 'Edit', action: handleEditWithPopulation }, { name: 'Delete', action: handleDelete });
  }
  return cols;
})
...
const populateFormFields = async (item) => {
  const countryField = branchFormFields.value.find(field => field.databaseField === 'country_id');
  const stateField = branchFormFields.value.find(field => field.databaseField === 'state_id');
  
  if (countryField) {
    countryField.value = item.country_id;
    await fetchStatesForCountry(item.country_id);
    if (stateField) {
      stateField.value = item.state_id;
    }
  }
};

const handleEditWithPopulation = async (item) => {
  handleEdit(item);
  await populateFormFields(item);
};
