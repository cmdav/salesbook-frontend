<template>
    <DashboardLayout pageTitle="Purchase page">
        <div class="container p-0 lg:p-6 lg:py-3 py-4 mb-5">
            <DataTableLayout @toggleModal="showPurchaseModal = !showPurchaseModal" :key="forceUpdate"
                endpoint="purchases" searchEndpoint="search-purchases" :additionalColumns="additionalColumns">
                    ...
                    <div v-for="(question, index) in formState.purchases" :key="index" class="flex flex-col gap-2">
                        ...
                        <div class="flex flex-col gap-2">
                            <div class="w-full">
                                ...
                                <select required v-model="formState.purchases[index].product_type_id"
                                    :label="`Product ${index + 1}`" :name="`Product ${index + 1}`"
                            ...
                            <div class="w-full">
                                ...
                                <div class="flex flex-row w-full gap-2">
                                    <div class="w-full">
                                        ...
                                        <select v-model="formState.purchases[index].supplier_id"
                                            :label="`Supplier ${index + 1}`" :name="`Supplier ${index + 1}`"
                                            :placeholder="`Add Supplier ${index + 1}`"
                                            class="w-full font-light font-Satoshi400 border-neutral-900 text-[14px] outline-none !p-[14px] border-[1px] opacity-[0.8029] rounded-[4px] text-sm">
                                            <option v-for="supplier in suppliersByProductId" :key="supplier.id"
                                                :value="supplier.id">
                                ...
                                <div class="flex flex-row w-full gap-2">
                                    <div class="w-full">
                                        ...
                                        <input required :label="`batch no ${index + 1}`" :name="`batch no ${index + 1}`"
                                            ...
                                            v-model="formState.purchases[index].batch_no" type="text"
                                            class="w-full font-light font-Satoshi400 border-neutral-900 text-[14px] outline-none !p-[14px] border-[1px] opacity-[0.8029] rounded-[4px] text-sm" />
                                    </div>
                                    <div class="w-full">
                                        <label class="block text-sm font-medium text-gray-700">
                                        ...
                                        <input required :label="`quantity ${index + 1}`" :name="`quantity ${index + 1}`"
                                            ...
                                            v-model="formState.purchases[index].quantity" min="0" type="number"
                                            class="w-full font-light font-Satoshi400 border-neutral-900 text-[14px] outline-none !p-[14px] border-[1px] opacity-[0.8029] rounded-[4px] text-sm" />
                                    </div>
                                </div>
                                <div class="flex flex-row w-full gap-2">
                                    <div class="w-full">
                                        ...
                                        <input :label="`product identifier ${index + 1}`"
                                    ...
                                    <div class="w-full">
                                        ...
                                        <input :label="`expired date ${index + 1}`" :name="`expired date ${index + 1}`"
                                            ...
                                            v-model="formState.purchases[index].expiry_date" type="date" :min="minDate"
                                            class="w-full font-light font-Satoshi400 border-neutral-900 text-[14px] outline-none !p-[14px] border-[1px] opacity-[0.8029] rounded-[4px] text-sm" />
                                    </div>
                                </div>
                            </div>
