...
<template>
  <div class="flex w-fit items-center gap-x-2">
    ...
    <button
      ...
      :key="number"
      :class="currentPage == number ? 'bg-brand text-white ring-brand' : 'text-brand ring-brand'"
    ...
    <button
      v-if="lastPage - currentPage >= 3"
      :class="currentPage == lastPage ? 'bg-brand text-white ring-brand' : 'text-brand ring-brand'"
