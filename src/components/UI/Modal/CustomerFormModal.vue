<template>
  ...
  <div class="grid grid-cols-2 gap-4">
    <div v-for="field in currentFormFields" :key="field.databaseField" class="mb-4">
      ...
      <span v-if="errors[field.databaseField]" class="text-red-600 text-sm">{{ errors[field.databaseField] }}</span>
    </div>
  </div>
...
const submitForm = async () => {
  ...
  currentFormFields.value.forEach(field => {
    const value = formData.value[field.databaseField];

    // Check if required field is empty
    if (field.required && !value) {
      errors.value[field.databaseField] = `${field.label} is required`;
    }
    
    // Check if the field should be a string (for specific fields)
    if (
      ['first_name', 'last_name', 'address', 'company_name', 'contact_person'].includes(field.databaseField) && 
      typeof value !== 'string'
    ) {
      errors.value[field.databaseField] = `${field.label} must be a string`;
    }

    // Max length validation
    if (
      ['first_name', 'last_name', 'company_name', 'contact_person'].includes(field.databaseField) && 
      value.length > 55
    ) {
      errors.value[field.databaseField] = `${field.label} must not exceed 55 characters`;
    }

    if (field.databaseField === 'address' && value.length > 150) {
      errors.value[field.databaseField] = 'Address must not exceed 150 characters';
    }

    // Email validation
    if (field.databaseField === 'email' && value) {
    ...
    if (field.databaseField === 'phone_number' && value) {
      const phonePattern = /^[0-9]{10,15}$/; // Simple pattern for phone number
      if (!phonePattern.test(value)) {
        errors.value.phone_number = 'Invalid phone number';
      }
    }
  });
